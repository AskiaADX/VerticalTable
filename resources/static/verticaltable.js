...
(function () {
  ...
  function removeClass(obj, clsName) {
    if (obj.classList) {
      ...
      obj.className = obj.className.replace(
        new RegExp("(^|\\b)" + clsName.split(" ").join("|") + "(\\b|$)", "gi"),
        " "
      );
    }
  }
  ...
  function checkAnswersDate(td) {
    ...
    if (inputDk) {
      if (inputDate && selectHour) {
        if (
          selectSecond &&
          inputDk.checked === false &&
          (inputDate.value === "" ||
            selectHour.value.toLowerCase() === "hh" ||
            selectMinute.value.toLowerCase() === "mm" ||
            selectSecond.value.toLowerCase() === "ss")
        )
          result = false;
        if (
          !selectSecond &&
  ...
  function checkAnswersOpen(td) {
    ...
    var result = true;
    if (inputDk) {
      if (inputDk.checked === true) inputOpen.value = "";
      if (inputDk.checked === false && inputOpen.value.trim().length < 2)
        result = false;
    } else {
  ...
  function checkAnswersNumeric(td) {
    ...
    var result = true;
    if (inputDk) {
      if (inputDk.checked === false && inputNumeric.value === "")
        result = false;
    } else {
  ...
  function checkAnswersClosed(td) {
    var inputsClosed = td.querySelectorAll(
  ...
  function stepByStepRows(that) {
    ...
    for (var i = trs.length; i-- > 0; ) {
      ...
      for (var j = tds.length; j-- > 1; ) {
        if (
          hasClass(tds[j], "date") &&
          (checkAnswersDate(tds[j]) ||
            that.arrInputCodesHiddenQuestions.indexOf(
              parseInt(tds[j].getAttribute("data-class").split("_")[1], 10)
            ) >= 0)
        ) {
          nbDataFound = nbDataFound + 1;
        } else if (
          hasClass(tds[j], "open") &&
          (checkAnswersOpen(tds[j]) ||
            that.arrInputCodesHiddenQuestions.indexOf(
              parseInt(tds[j].getAttribute("data-class").split("_")[1], 10)
            ) >= 0)
        ) {
          ...
          hasClass(tds[j], "numeric") &&
          (checkAnswersNumeric(tds[j]) ||
            that.arrInputCodesHiddenQuestions.indexOf(
              parseInt(tds[j].getAttribute("data-class").split("_")[1], 10)
            ) >= 0)
        ) {
          ...
          hasClass(tds[j], "select") &&
          (checkAnswersSelect(tds[j]) ||
            that.arrInputCodesHiddenQuestions.indexOf(
              parseInt(tds[j].getAttribute("data-class").split("_")[1], 10)
            ) >= 0)
        ) {
          ...
          hasClass(tds[j], "closed") &&
          (checkAnswersClosed(tds[j]) ||
            that.arrInputCodesHiddenQuestions.indexOf(
              parseInt(tds[j].getAttribute("data-class").split("_")[1], 10)
            ) >= 0)
        ) {
      ...
      if (i === trs.length - 1 && nbDataFound - nbHiddenQuestions === 0)
        continue;
      // Check if all questions have answers
      if (nbDataFound !== tds.length - 1 && lastDataFound === 0) {
        if (that.stepByStep && that.useStepByStep === "auto")
          trs[i + 1].style.display = "none";
        if (that.stepByStep && that.useStepByStep === "buttons")
          document
            .querySelector(
              "#adc_" + that.instanceId + " .plusbutton input[type=button]"
            )
        ...
        if (hasClass(trs[i + 1], "display")) {
          if (
            window.innerWidth <= that.responsiveWidth &&
            that.scrollNextIteration &&
            !(that.stepByStep && that.useStepByStep === "buttons")
          )
    ...
    document
      .querySelectorAll("#adc_" + that.instanceId)[0]
      .classList.remove("hideatstart"); // make the table visible
  }
  ...
  function stepByStepButtons(that) {
    ...
    for (var i = trs.length; i-- > 0; ) {
      ...
      if (that.stepByStep && that.useStepByStep === "buttons") {
        ...
        if (
          nbDataFound !== tds.length - 1 &&
          nbDataFound - nbHiddenQuestions === 0 &&
          i > 0
        ) {
  ...
  function clearMultiple(td) {
    ...
    if (labelsSelected.length > 0) {
      for (var i = 0, j = labelsSelected.length; i < j; i++) {
        var checkbox = labelsSelected[i].querySelector(
          'input[type="checkbox"]'
        );
  ...
  function manageExclusive(obj) {
    ...
    for (var i = 0; (j = responsesContainer.children.length), i < j; i++) {
      if (
        responsesContainer.children[i].children[0] !== obj &&
        responsesContainer.children[i].className.indexOf("selected") >= 0 &&
        (obj.className.indexOf("askia-exclusive") >= 0 ||
          obj.type === "radio" ||
          responsesContainer.children[i].children[0].className.indexOf(
            "askia-exclusive"
          ) >= 0)
      ) {
        ...
        if (obj.type === "checkbox") {
          document.getElementById(
            responsesContainer.children[i].children[0].attributes.id.value
          ).checked = false;
        }
      }
    }
  ...
  function onChange(event, that) {
    ...
    if (
      ...
      if (el.className !== "dkbutton") {
        var shortcut =
          that.questions[
            parseInt(el.getAttribute("data-class").split("_")[1], 10) - 1
          ] || "";
  ...
  function onNumericInputDK(event, that) {
    ...
    var inputNumber = document.getElementById(
      "askia-input-number" + inputValue
    );
    var inputRange = document.getElementById("askia-input-range_" + inputValue);
    var inputCurrency = document.getElementById(
      "askia-input-currency" + inputValue
    );
    if (el.nodeName === "INPUT" && el.type === "checkbox") {
      if (el.checked) {
        ...
        if (that.useSlider === 1) {
          var suffix =
            that.suffixes[parseInt(inputRange.className.split("_")[2], 10) - 1];
          inputRange.value = "";
  ...
  function onInputNumbers(event, that) {
    ...
    var sum = 0;
    var inputNumbers = document.querySelectorAll(
      "#adc_" + that.instanceId + " ." + el.className
    );
    for (var i = 0; i < inputNumbers.length; i++) {
      sum += parseFloat(inputNumbers[i].value) || 0;
    }
    var results = document.querySelectorAll(
      "#adc_" + that.instanceId + " .result_" + split[1] + "_" + split[2]
    );
    var resultSum = document.querySelectorAll(
      "#adc_" + that.instanceId + " .sum_" + split[1] + "_" + split[2]
    );
    for (var j = 0; j < results.length; j++) {
      // Update the live sum result
      if (that.showTotal === 1) {
        results[j].innerHTML = sum.toLocaleString(undefined, {
          minimumFractionDigits: decimals,
          maximumFractionDigits: decimals,
        });
      } else if (that.showTotal === 2) {
        if (isNaN(maxLimit) === false) {
          results[j].innerHTML = (maxLimit - sum).toLocaleString(undefined, {
            minimumFractionDigits: decimals,
            maximumFractionDigits: decimals,
          });
        } else {
      ...
      if (
        ...
        (that.showTotal === 1 || that.showTotal === 2)
      ) {
        if (sum === maxLimit) {
  ...
  function onInputRanges(event, that) {
    ...
    var decimals = that.decimals[parseInt(split[2], 10) - 1] || 0;
    var inputNumber = document.querySelector(
      "#adc_" + that.instanceId + " #askia-input-number" + el.id.split("_")[1]
    );
    inputNumber.value = el.value;
    el.parentElement.nextElementSibling.innerHTML =
      parseFloat(el.value).toLocaleString(undefined, {
        minimumFractionDigits: decimals,
        maximumFractionDigits: decimals,
      }) + suffix;
    addClass(el, "selected");
    document.querySelector(
      "#adc_" + that.instanceId + " #" + el.id + " + .preBar"
    ).style.width = widthRange(el) + "px";
    if ("createEvent" in document) {
      var evt = document.createEvent("HTMLEvents");
      evt.initEvent("input", false, true);
      inputNumber.dispatchEvent(evt);
  ...
  function headerFix(el, opt) {
    ...
    var offsetHeight =
      document.getElementById("adc_" + opt.instanceId).offsetHeight ||
      document.getElementById("adc_" + opt.instanceId).height;
    var offsetHeightThead =
      document.querySelector("#adc_" + opt.instanceId + " thead")
        .offsetHeight ||
      document.querySelector("#adc_" + opt.instanceId + " thead").height;
    var offsetTop = calcOffsetTop(
      document.getElementById("adc_" + opt.instanceId)
    );
    var scrollTop =
    ...
    if (
      scrollTop >= offsetTop &&
      scrollTop <= offsetTop + offsetHeight - (offsetHeightThead + 10)
    ) {
  ...
  function simplboxConstructorCall(strId) {
    ...
    var img = new SimplBox(
      document.querySelectorAll("[data-simplbox='" + strId + "']"),
      {
    ...
    for (i = 0; i < inputSelect.length; i++) {
      ...
      if (selectedIndex !== 0) {
        for (j = i + 1; j < inputSelect.length; j++) {
          ...
          if (inputSelect[j].selectedIndex === selectedIndex)
            inputSelect[j].selectedIndex = 0;
  ...
  function numberWithThousandSeparator(x) {
    ...
    var parts = [];
    var inputNumber = document.querySelector(
      "[name=" + x.name.toString().split("_")[1] + "]"
    );
    ...
    if (currentValue.length < x.value.length) {
      setTimeout(function () {
        setCaretPosition(x, currentPosition.start + 1, currentPosition.end + 1);
      }, 1);
  ...
  function VerticalTable(options) {
    ...
    addEvent(
      ...
      (function (passedInElement) {
        return function (data) {
          var indexInputCode =
            passedInElement.arrInputCodesHiddenQuestions.indexOf(
              data.detail.question.inputCode
            );
          if (indexInputCode !== -1) {
            passedInElement.arrInputCodesHiddenQuestions.splice(
              indexInputCode,
              1
            );
          }
    ...
    addEvent(
      ...
      (function (passedInElement) {
        return function (data) {
          ...
          if (indexInputCode === -1) {
            passedInElement.arrInputCodesHiddenQuestions.push(
              data.detail.question.inputCode
            );
          }
          var debounceStepByStep = debounce(stepByStepRows, 300);
    ...
    var radios = document.querySelectorAll(
      "#adc_" + this.instanceId + ' input[type="radio"]'
    );
    var checkboxes = document.querySelectorAll(
      "#adc_" + this.instanceId + ' input[type="checkbox"]'
    );
    var inputNumbers = document.querySelectorAll(
      "#adc_" + this.instanceId + ' input[type="number"]'
    );
    var inputRanges = document.querySelectorAll(
      "#adc_" + this.instanceId + ' input[type="range"]'
    );
    var numInputDK = document.querySelectorAll(
      "#adc_" + this.instanceId + ' .numeric .DK input[type="checkbox"]'
    );
    var inputOpens = document.querySelectorAll(
      "#adc_" + this.instanceId + " .open .inputopen"
    );
    var openInputDK = document.querySelectorAll(
      "#adc_" + this.instanceId + ' .open .DK input[type="checkbox"]'
    );
    var inputDates = document.querySelectorAll(
      "#adc_" + this.instanceId + ' .date input[type="text"]'
    );
    var dateInputDK = document.querySelectorAll(
      "#adc_" + this.instanceId + ' .date .DK input[type="checkbox"]'
    );
    ...
    for (var l = 0; l < inputRanges.length; l++) {
      ...
      document.querySelector(
        "#adc_" + this.instanceId + " #" + inputRanges[l].id + " + .preBar"
      ).style.width = widthRange(inputRanges[l]) + "px";
    }
      ...
      for (var l2 = 0; l2 < inputRanges.length; l2++) {
        document.querySelector(
          "#adc_" +
            ...
            inputRanges[l2].id +
            " + .preBar"
        ).style.width = widthRange(inputRanges[l2]) + "px";
      }
    });
    ...
    for (var i1 = 0; i1 < this.rankingBox.length; i1++) {
      if (this.rankingBox[i1]) {
        var inputElt = document.querySelectorAll(
          ".select_" + this.instanceId + "_" + (i1 + 1)
        );
    ...
    for (var i2 = 0; i2 < this.selectBox.length; i2++) {
      if (this.selectBox[i2]) {
        var selectElt = document.querySelectorAll(
          ".select_" + this.instanceId + "_" + (i2 + 1)
        );
    ...
    if (this.useSlider === 2) {
      ...
      document.addEventListener(
        ...
        function (event) {
          ...
          if (
            el.nodeName === "INPUT" &&
            el.className.indexOf("thousand") >= 0
          ) {
    ...
    var ths = document.querySelectorAll(
      "#adc_" + options.instanceId + " thead th"
    );
    ...
    // Manage add button
    var buttonAdd = document.querySelector(
      "#adc_" + options.instanceId + ' .plusbutton input[type="button"]'
    );
    var buttonsRemove = document.querySelectorAll(
      "#adc_" + options.instanceId + ' .minusbutton input[type="button"]'
    );
    ...
    // Manage zoom
    var zooms = document
      .getElementById("adc_" + this.instanceId)
